...
void Board::placePiecesOnSquare(Piece piece,Color color)
{
	switch (piece)
	{
...
void Board::printBoard() {
	...
	for (int i = 0; i < 8; i++)
	{
		...
		for (int j = 0; j < 8; j++)
		{
			...
			placePiecesOnSquare(piece,color);
		}
		cout << endl;
	}
...
bool Board::hasMoveBeenDone() {
	...
	while (!stop)
	{
		...
		if (getSquare(x1, y1)->getColor() == turn)
			if (hasMoveBeenMade(x1, y1, x2, y2) == false) ? cout << "Invalid move, try again." << endl : stop = true;
			else
...
bool Board::hasGameBeenPlayed()
{
	system("cls");
	printBoard();
	return hasMoveBeenDone();

}

bool Board::hasKingBeenMoved(Square* thisKing, Square* thatSpace) {
...
bool Board::hasQueenBeenMoved(Square* thisQueen, Square* thatSpace) { //there might be problems with numbers of brackets
													   //off board inputs should be handled elsewhere (before this)
													   //squares with same color should be handled elsewhere (before this)
...
bool Board::hasBishopBeenMoved(Square* thisBishop, Square* thatSpace) { //there might be problems with number of brackets
...
bool Board::hasKnightBeenMoved(Square* thisKnight, Square* thatSpace)
{
	//off board inputs should be handled elsewhere (before this)
	//squares with same color should be handled elsewhere (before this)
...
bool Board::hasRookBeenMoved(Square* thisRook, Square* thatSpace)
{
	//off board inputs should be handled elsewhere (before this)
	//squares with same color should be handled elsewhere (before this)
...
bool Board::hasPawnBeenMoved(Square* thisPawn, Square* thatSpace) {
	//off board inputs should be handled elsewhere (before this)
	//squares with same color should be handled elsewhere (before this)
...
bool Board::hasPieceBeenMoved(Square* src, Square* dest)
{
	Piece piece = src->getPiece();
	switch (piece)
	{
	case KING: return hasKingBeenMoved(src, dest);
		break;
	case QUEEN: return hasQueenBeenMoved(src, dest);
		break;
	case BISHOP: return hasBishopBeenMoved(src, dest);
		break;
	case KNIGHT: return hasKnightBeenMoved(src, dest);
		break;
	case ROOK: return hasRookBeenMoved(src, dest);
		break;
	case PAWN: return hasPawnBeenMoved(src, dest);
		break;
	case EMPTY: std::cout << "You do not have a piece there" << std::endl;  return false;
		break;
	default: std::cerr << "Something went wrong in the switch statement in hasMoveBeenMade()" << std::endl;
		break;
	}
}
bool Board::hasMoveBeenMade(int x1, int y1, int x2, int y2) {
